...
from utils.evaluation_utils import decode, post_processing, draw_predictions, \
                                   convert_det_to_real_values, convert_detection_to_kitti_annos
...
def parse_test_configs():
    ...
    parser.add_argument('--trt_path', type=str,
                        default='../checkpoints/trt/model3d_fp16.engine',
...
if __name__ == '__main__':
    ...
    onnx_model = onnx.load(configs.onnx_path)
    input_name = "images"
    shape_dict = {input_name: (1,3,608,608)}
    target="cuda"
    mod, params = relay.frontend.from_onnx(onnx_model, shape_dict)
    with autotvm.apply_history_best('./results/tvm_tuning_results.json'):
    ...
    print(f"Model loaded in {model_load_end - model_load_start}s")
    ...
    for batch_idx, batch_data in enumerate(test_dataloader):
        ...
        input_img = bev_maps.float().numpy()
        start = time.time()
        module.set_input(input_name, input_img)
        module.run()
        end = time.time()
        infer_time = end - start
        outputs = []
        outputs.append(torch.from_numpy(
            module.get_output(0, tvm.nd.empty((1, 3, 152, 152))).numpy()).float())
        outputs.append(torch.from_numpy(
            module.get_output(1, tvm.nd.empty((1, 2, 152, 152))).numpy()).float())
        outputs.append(torch.from_numpy(
            module.get_output(2, tvm.nd.empty((1, 2, 152, 152))).numpy()).float())
        outputs.append(torch.from_numpy(
            module.get_output(3, tvm.nd.empty((1, 1, 152, 152))).numpy()).float())
        outputs.append(torch.from_numpy(
            module.get_output(4, tvm.nd.empty((1, 3, 152, 152))).numpy()).float())
        ...
        if configs.save_test_output:
            if configs.output_format == 'image':
                ...
                raise TypeError
    # dt_annos = convert_detection_to_kitti_annos(detection_list, test_dataloader.dataset)
       
    # with open('dt_annos_tensorrt_rtx_fp16.pickle', 'wb') as handle:
    #     pickle.dump(dt_annos, handle, protocol=pickle.HIGHEST_PROTOCOL)
    # fpn_resnet.destroy()
    print(f"total_time= {time.time() - total_start_time}")
